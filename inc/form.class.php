...
class PluginMetademandsForm extends CommonDBTM {
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      ...
      }else if ($item->getType() == 'PluginResourcesResource') {
         if ($this->canView()
             && !$withtemplate
             && countElementsInTable("glpi_plugin_metademands_forms", ["resources_id" => $item->fields['id']])) {
            $form_metademand_data = $this->find(['resources_id' => $item->fields['id']]);
            $total                = count($form_metademand_data);
            $name                 = _n('Associated form', 'Associated forms', $total, 'metademands');
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0) {
      ...
      switch ($item->getType()) {
         ...
         case 'PluginResourcesResource':
            $form->showFormsForResource($item);
            break;
      }
   ...
   function showFormsForUser($user) {
      ...
      if (count($forms_metademands)) {
         ...
         foreach ($forms_metademands as $forms_metademand) {
            ...
            $resources_id                          = $forms_metademand['resources_id'];
            echo "<tr class='tab_bg_1'>";
            echo "<td>";
            $meta = new PluginMetademandsMetademand();
            $meta->getFromDB($plugin_metademands_metademands_id);
   ...
   function showFormsForResource($resource) {

      if (!$this->canView()) {
         return false;
      }
      $forms_metademands = $this->find(['resources_id' => $resource->fields['id']]);
      ...
      if (count($forms_metademands)) {
         ...
         foreach ($forms_metademands as $forms_metademand) {
            ...
            $plugin_metademands_metademands_id = $forms_metademand['plugin_metademands_metademands_id'];
            $users_id                          = $forms_metademand['users_id'];
            $items_id                          = $forms_metademand['items_id'];
            $itemtype                          = $forms_metademand['itemtype'];
            $resources_id                          = $forms_metademand['resources_id'];
            echo "<tr class='tab_bg_1'>";
            ...
            echo "<td>";
            echo User::getFriendlyNameById($forms_metademand['users_id']);
            echo "</td>";
