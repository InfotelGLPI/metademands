 ...
 *
 **/
class MetademandTranslation extends CommonDBChild
{
    ...
    public static function install(Migration $migration)
    {
        ...
        $query = $DB->buildUpdate(
            ...
            [
                'itemtype' => 'PluginMetademandsField'
            ]
        );
        $DB->doQuery($query);
    ...
    public static function showTranslations($item)
    {
        ...
        if ($canedit) {
            echo "<div id='viewtranslationmeta" . $item->getID() . "$rand'></div>\n";

            echo "<script type='text/javascript' >\n";
            echo "function addTranslationmeta" . $item->getID() . "$rand() {\n";
            $params = ['type'                      => __CLASS__,
                'parenttype'                => get_class($item),
                $item->getForeignKeyField() => $item->getID(),
                'id'                        => -1];
            Ajax::updateItemJsCode(
                "viewtranslationmeta"  . $item->getID() . "$rand",
            ...
            echo "<div class='center'>"
              . "<a class='submit btn btn-primary' href='javascript:addTranslationmeta"
            ...
            foreach ($iterator as $data) {
                ...
                if ($canedit) {
                    $onhover = "style='cursor:pointer'
                           onClick=\"viewEditTranslationmeta" . $data['id'] . "$rand();\"";
                }
                ...
                if ($canedit) {
                    ...
                    echo "function viewEditTranslationmeta" .  $data['id'] . "$rand() {\n";
                    $params = ['type'                      => __CLASS__,
                        'parenttype'                => get_class($item),
                        $item->getForeignKeyField() => $item->getID(),
                        'id'                        => $data["id"]];
    ...
    public function showForm($ID = -1, $options = [])
    {
        ...
        if ($ID > 0) {
            ...
            Ajax::updateItemOnSelectEvent(
                ...
                "span_fields",
                PLUGIN_METADEMANDS_WEBDIR . "/ajax/updateTranslationMetademands.php",
                $params
            );
        }
    ...
    public static function canBeTranslated(CommonGLPI $item)
    {
        ...
        return ($item instanceof Metademand);
    }
