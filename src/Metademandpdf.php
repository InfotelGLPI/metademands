...
class MetaDemandPdf extends Fpdf
{
    ...
    function Header()
    {
        ...
        $image = '../public/pics/login_logo_glpi.png';
        $target = 20;
    ...
    function MultiCellValue($w, $h, $type, $label, $values, $border = 'LRB', $align = 'C', $color = '', $bold = false, $size = 10, $fontColor = '', $link = '')
    {
        ...
        } elseif ($type == 'title' || $type == 'title-block' || $type == 'textarea' || $type == 'signature' || $type == 'freetable') {
            $this->MultiCell($this->title_width, $h, $label, $border, $align, true);
        } else {
        ...
        for ($i = 0; $i < $nb; $i++) {
            ...
            foreach ($newForm as $key => $elt) {
                ...
                if ((isset($fields[$elt['id']])
                    ...
                    $label = "";
                    if (!empty($elt['name'])) {
                        if (empty($label = Field::displayField($elt['id'], 'name'))) {
                            $label = $elt['name'];
                        }
                    ...
                    if (in_array($elt['type'], $allowed_customvalues_types)
                        || (isset($elt['item']) && in_array($elt['item'], $allowed_customvalues_items))
                    ) {
                    ...
                    switch ($elt['type']) {
                        ...
                        case 'dropdown_meta':
                            ...
                            switch ($elt['item']) {
                                case 'User':
                                    ...
                                    if (empty($dataItems)) {
                                        $value = getUserName($fields[$elt['id']], 0, true);
                                    } else {
                                        $value = $dataItems;
                                    }
                                ...
                                case 'other':
                                    if (!empty($elt['custom_values']) && isset($elt['custom_values'])) {
                                        $custom_values = [];
                                        foreach ($elt['custom_values'] as $key => $val) {
                        ...
                        case 'checkbox':
                            ...
                            if (!empty($elt['custom_values'])) {
                                ...
                                $values = FieldParameter::_unserialize($fields[$elt['id']]);
                                $custom_checkbox = [];
                        ...
                        case 'date_interval':
                            ...
                            if (!empty($elt['label2'])) {
                                ...
                                if (empty($label2 = Field::displayField($elt['id'], 'label2'))) {
                                    $label2 = $elt['label2'];
                                }
                        ...
                        case 'basket':
                            if (Plugin::isPluginActive('orderfollowup')) {
                                ...
                                if ($ordermaterialmeta->getFromDBByCrit(['plugin_metademands_metademands_id' => $elt['plugin_metademands_metademands_id']])) {
                                    ...
                                    $value = Basket::displayFieldPDF($elt, $fields, $label);
                                    $this->MultiCellValue($this->value_width, $this->line_height, $elt['type'], $label, $value, 'LRBT', 'L', '', 0, '', 'black');
                                }
                            } else {
                                $value = Basket::displayFieldPDF($elt, $fields, $label);
