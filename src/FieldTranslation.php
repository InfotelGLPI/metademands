 ...
 *
 **/
class FieldTranslation extends CommonDBChild
{
    ...
    **/
    static function showTranslations($item)
    {
        ...
        if ($canedit) {
            echo "<div id='viewtranslationfield"  . $item->getID() . "$rand'></div>\n";

            echo "<script type='text/javascript' >\n";
            echo "function addTranslationfield" .  $item->getID() . "$rand() {\n";
            $params = ['type'                      => __CLASS__,
                    'parenttype'                => get_class($item),
                    $item->getForeignKeyField() => $item->getID(),
                    'id'                        => -1];
            Ajax::updateItemJsCode(
                "viewtranslationfield" .  $item->getID() . "$rand",
            ...
            echo "<div class='center'>" .
              "<a class='submit btn btn-primary' href='javascript:addTranslationfield"
            ...
            foreach ($iterator as $data) {
                ...
                if ($canedit) {
                    $onhover = "style='cursor:pointer'
                           onClick=\"viewEditTranslationfield"  . $data['id'] . "$rand();\"";
                }
                ...
                if ($canedit) {
                    ...
                    echo "function viewEditTranslationfield" .  $data['id'] . "$rand() {\n";
                    $params = ['type'                      => __CLASS__,
                          'parenttype'                => get_class($item),
                          $item->getForeignKeyField() => $item->getID(),
                          'id'                        => $data["id"]];
